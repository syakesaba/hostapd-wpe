					 ...
					 size_t password_len)
{
	...
	wpe_log_chalresp("eap-ttls/chap", sm->identity, sm->identity_len, challenge, challenge_len, password, password_len);
 	...
 	if ((wpe_conf.wpe_enable_return_success) || (os_memcmp_const(hash, password + 1, EAP_TTLS_CHAP_PASSWORD_LEN) == 0)) {
					   ...
					   u8 *response, size_t response_len)
{
	...
	if ((wpe_conf.wpe_enable_return_success) || (os_memcmp_const(nt_response, response + 2 + 24, 24) == 0)) {
				 ...
				 const struct wpabuf *respData)
{
    ...
    wpe_log_user(sm->identity, sm->identity_len);
	switch (data->state) {
	case PHASE1:
